...
namespace fooTitle.Layers
{
    ...
    public class FillImagesLayer : Layer
    {
        ...
        public FillImagesLayer(Rectangle parentRect, XElement node, Skin skin)
        ...
        protected void AddImage(XPathNavigator node)
        {
            ...
            Bitmap b = ScaleImage(ParentSkin.GetSkinImage(src));
        ...
        private void PrepareRepeatImage()
        {
            ...
            if (_repeatCenter)
            {
                ...
                {
                    _centerRepeatedCanvas.DrawImage(_centerImage, new Rectangle((int)Math.Round((float)(i * _centerImage.Width)), 0, (int)Math.Round((float)_centerImage.Width), ClientRect.Height), 0, 0, _centerImage.Width, _centerImage.Height, GraphicsUnit.Pixel, attrs);
                }
            }
            else
            {
        ...
        protected override void DrawImpl(Graphics canvas)
        {
            ...
            if (_leftImage != null)
            {
                canvas.DrawImage(_leftImage, ClientRect.X, ClientRect.Y, _leftImage.Width, ClientRect.Height);
            }

            canvas.DrawImage(_centerRepeated, ClientRect.X + left, ClientRect.Y, _centerRepeated.Width, ClientRect.Height);

            if (_rightImage != null)
            {
                canvas.DrawImage(_rightImage, ClientRect.X + right, ClientRect.Y, _rightImage.Width, ClientRect.Height);
            }
        }
        ...
        {
            Rectangle destRect = new Rectangle(0, 0, DpiHandler.ScaleValueByDpi(image.Width), DpiHandler.ScaleValueByDpi(image.Height));
            Bitmap destImage = new Bitmap(DpiHandler.ScaleValueByDpi(image.Width), DpiHandler.ScaleValueByDpi(image.Height));

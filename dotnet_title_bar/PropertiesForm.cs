...
namespace fooTitle
{
    class PropertiesForm : UserControl
    {
        ...
        public PropertiesForm(Properties parent)
        {
            ...
            showWhenWrapper = new RadioGroupWrapper("showControl/enableWhen", preferencesCallback);
            ...
            windowPositionWrapper = new RadioGroupWrapper("display/windowPosition", preferencesCallback);
            ...
            popupShowingWrapper = new RadioGroupWrapper("showControl/popupShowing", preferencesCallback);
            popupShowingWrapper.AddRadioButton(allTheTimeRadio);
            popupShowingWrapper.AddRadioButton(onlyTriggerRadio);

            enableDraggingWrapper = new RadioGroupWrapper("display/enableDragging", preferencesCallback);
        ...
        private async Task<List<ListViewItem>> GetSkinItems(CancellationToken token)
        {
            ...
            {
                Main.Console.LogError($"Failed to read from {Main.UserDataDir}:\n{e}");
    ...
    " that starts loading art after the song\r\nhas already started playing.";
        ...
        private void openSkinDirBtn_Click(object sender, EventArgs e)
        {
            try
            {
                if (skinsList.SelectedItems.Count == 0)
                {
                    var psi = new System.Diagnostics.ProcessStartInfo() { FileName = Main.UserDataDir, UseShellExecute = true };
